<template lang="pug">
.curso-main-container.pb-3
  BannerInterno
  .container.tarjeta.tarjeta--blanca.p-4.p-md-5
    
    .titulo-principal.color-acento-contenido
      .titulo-principal__numero
        span 2
      h1 Confección de prendas femeninas

    .row.justify-content-center(data-aos="zoom-in-left")
      .col-sm-12.col-lg-10
        .bloque-texto-g.color-primario.p-3.p-sm-4.p-md-5.mb-5
          .bloque-texto-g__img.bgimg02
          .bloque-texto-g__texto.p-4
            p En el proceso de confección o ensamble de prendas masculinas y femeninas desde el que hacer de un modisto o sastre, se requiere el manejo y dominio básico de las máquinas de coser y sus aditamentos en el momento de elaborar cualquier tipo de prenda, si bien la destreza en el manejo es importante, se debe tener claro que todo molde, patrón, plano o desarrollo de diseño se acompaña de una ruta u orden operacional como especificaciones para el ensamble, las cuales van de la mano con el área de prototipado o muestreo.

    Separador  
    #t_2_1.titulo-segundo.color-primario(data-aos="zoom-in-left")
      h2 2.1 Trazo básico blusa

    p.mb-5 Es necesario que para la confección de la falda clásica se tengan los elementos que fueron previamente trazados y patronados, así como el material textil, cierre e insumos necesarios para generar los acabados. 

    .row.justify-content-center.mb-5
      .col-sm-12.col-lg-2
        img.img-a(src='@/assets/curso/tema_2/img_2.png', alt='falda')
      .col-sm-12.col-lg-4
        .fw-bold.mb-5 Ruta operacional de la falda
        p.mb-4 La elaboración de una falda cuenta con lo siguientes pasos:
        ol.lista-ol
          li
            .lista-ol--cuadro__vineta.color-primario
              span.text-bold 1. 
            | Unir los costados de la falda con maquina plana y posteriormente pasarla por la fileteadora.

          li
            .lista-ol--cuadro__vineta.color-primario 
              span.text-bold 2. 
            | Dejar abierto el centro de espalda para colocar el cierre con la pretina.
          li
            .lista-ol--cuadro__vineta.color-primario 
              span.text-bold 3. 
            | Unir la pretina, en este caso es una pretina recta con corte a los costados y corte centro espalda, las cuales deberán estar previamente fusionadas con el interlon.
          li
            .lista-ol--cuadro__vineta.color-primario 
              span.text-bold 4. 
            | Pegar la pretina a la falda.
          li
            .lista-ol--cuadro__vineta.color-primario 
              span.text-bold 5. 
            | Colocar el cierre (invisible o normal).
          li
            .lista-ol--cuadro__vineta.color-primario 
              span.text-bold 6. 
            | Cerrar el centro de la espalda.
          li
            .lista-ol--cuadro__vineta.color-primario 
              span.text-bold 7. 
            | Realizar el dobladillo.
              

    .row.justify-content-center(data-aos="zoom-in-left")
      .col-sm-12.col-lg-8        
        p.mb-5 A continuación se presentarán algunos videos de apoyo para el alistamiento de recursos para la elaboración de la falda.


    .row.justify-content-center
      .col-sm-12.col-lg-8
        .tarjeta.color-acento-contenido.p-3.mb-5
          .row.justify-content-around.align-items-center
            .col-sm-12.col-lg-2.d-none.d-lg-block
              img(src="@/assets/template/icono-video.svg")
            .col-sm-12.col-lg-10
              .row.justify-content-between.align-items-center
                .col.mb-3.mb-sm-0
                  p Introducción trazo, corte y confección de la falda clásica
                .col-sm-auto
                  a.boton.color-boton(href="https://www.youtube.com/embed/liB1zqJfBqI" target="_blank").mb-1
                    span Ir al video
                    i.fas.fa-video  
              .row.justify-content-between.align-items-center
                .col.mb-3.mb-sm-0
                  p Elaboración del patrón frente de la falda clásica
                .col-sm-auto
                  a.boton.color-boton(href="https://www.youtube.com/embed/IPd6XzOf504" target="_blank").mb-1
                    span Ir al video
                    i.fas.fa-video
              .row.justify-content-between.align-items-center
                .col.mb-3.mb-sm-0
                  p Elaboración del patrón posterior de la falda clásica
                .col-sm-auto
                  a.boton.color-boton(href="https://www.youtube.com/embed/lFwR6RcFwIg" target="_blank").mb-1
                    span Ir al video
                    i.fas.fa-video
              .row.justify-content-between.align-items-center
                .col.mb-3.mb-sm-0
                  p Introducción: marcaciones sobre la tela y corte de la falda clásica
                .col-sm-auto
                  a.boton.color-boton(href="https://www.youtube.com/embed/typAdUPaeSE" target="_blank").mb-1
                    span Ir al video
                    i.fas.fa-video
              .row.justify-content-between.align-items-center
                .col.mb-3.mb-sm-0
                  p Despiece de la falda y marcaciones
                .col-sm-auto
                  a.boton.color-boton(href="https://www.youtube.com/embed/QVhz-5uv97o" target="_blank").mb-1
                    span Ir al video
                    i.fas.fa-video
              .row.justify-content-between.align-items-center
                .col.mb-3.mb-sm-0
                  p Alistamiento de maquinaria cumpliendo normas de seguridad
                .col-sm-auto
                  a.boton.color-boton(href="https://www.youtube.com/embed/AZAY9W7RLwI" target="_blank").mb-1
                    span Ir al video
                    i.fas.fa-video     
              .row.justify-content-between.align-items-center
                .col.mb-3.mb-sm-0
                  p Introducción: confección de la falda clásica según normas de calidad
                .col-sm-auto
                  a.boton.color-boton(href="https://www.youtube.com/embed/aRcvWsb5mUs" target="_blank").mb-1
                    span Ir al video
                    i.fas.fa-video 
    Separador  
    #t_2_2.titulo-segundo.color-primario(data-aos="zoom-in-left")
      h2 2.2 Confección de complementos     

    p.mb-5 En la confección de complementos se encuentran diversos insumos que aportan a la prenda, soportes, ajustes, volumen, estructura, entre otros, por ello, se realizará un recorrido sobre la confección de prendas de vestir. 

    .row.justify-content-center
      .col-sm-12.col-lg-7
        .tarjeta.color-acento-contenido.p-3
          .row.justify-content-around.align-items-center
            .col-sm-12.col-lg-2.d-none.d-lg-block
              img(src="@/assets/template/icono-pdf.svg")
            .col-sm-12.col-lg-10
              .row.justify-content-between.align-items-center
                .col.mb-3.mb-sm-0
                  p.mb-0 Anexo. Las cremalleras.
                  p Conoce el proceso de confección de las cremalleras.
                .col-sm-auto
                  a.boton.color-boton(href="https://www.youtube.com/embed/" target="_blank").mb-1
                    span Ir al PDF
                    i.fas.fa-file-pdf  
              .row.justify-content-between.align-items-center
                .col.mb-3.mb-sm-0
                  p.mb-0 Anexo. Los puños.
                  p Conoce el proceso de confección de los puños.
                .col-sm-auto
                  a.boton.color-boton(href="https://www.youtube.com/embed/" target="_blank").mb-1
                    span Ir al PDF
                    i.fas.fa-file-pdf 
              .row.justify-content-between.align-items-center
                .col.mb-3.mb-sm-0
                  p.mb-0 Anexo. Los cuellos.
                  p Conoce el proceso de confección de los cuellos.
                .col-sm-auto
                  a.boton.color-boton(href="https://www.youtube.com/embed/" target="_blank").mb-1
                    span Ir al PDF
                    i.fas.fa-file-pdf 

</template>

<script>
export default {
  name: 'Tema2',
  data: () => ({
    // variables de vue
  }),
}
</script>

<style lang="sass" scoped></style>
